...
/**
 * 创 建 者: LoyBin
 * 创建时间: 2017/09/07 下午3:33
 * 描   述: 国学诗词Fragment
 */
public class PoetryFramgent extends BaseFragment implements AdapterView.OnItemClickListener, AbsListView.OnScrollListener, SpringView.OnFreshListener {
    ...
    public LoadingPager.LoadedResult initData(LoadingPager loadingPager) {
        ...
        if (mStoryBooksPresenter == null) {
            mStoryBooksPresenter = new StoryBooksPresenter(getContext(), this, "Poetry");
        }
        try {

            mAlbums = mStoryBooksPresenter.loadData(mPage, "841");
    ...
    public View initSuccessView() {
        try {
            ...
            mTotalCount = (int) SharedPreferencesUtils.getParam(MyApplication.sInstance, "Poetry_totalCount", 0);
            initListener();
            mPage++;
    ...
    public void onSuccess(List<Album> alba, int totalCount) {
        ...
        SharedPreferencesUtils.setParam(MyApplication.sInstance, "Poetry_totalCount", mTotalCount);
        if (mAlbums == null) {
            //去更新fragmen UI
            mAlbums = alba;
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        if (scrollState == SCROLL_STATE_IDLE) {
            ...
            if (view.getLastVisiblePosition() > count && (mAlbums == null
                ...
                mStoryBooksPresenter.loadDataNet(mPage, "841");
    ...
    public void onResume() {
        ...
        MobclickAgent.onPageStart("PoetryFramgent"); //统计页面，"MainScreen"为页面名称，可自定义
    }

    public void onPause() {
        super.onPause();
        MobclickAgent.onPause(getActivity());
        MobclickAgent.onPageEnd("PoetryFramgent");
    }
}
