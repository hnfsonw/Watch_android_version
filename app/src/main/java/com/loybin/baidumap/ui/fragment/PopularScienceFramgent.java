...
/**
 * 创 建 者: LoyBin
 * 创建时间: 2017/09/07 下午3:36
 * 描   述: 科普知识Fragment
 */
public class PopularScienceFramgent extends BaseFragment implements AdapterView.OnItemClickListener, AbsListView.OnScrollListener, SpringView.OnFreshListener {
    ...
    public LoadingPager.LoadedResult initData(LoadingPager loadingPager) {
        ...
        if (mStoryBooksPresenter == null) {
            mStoryBooksPresenter = new StoryBooksPresenter(getContext(), this, "PopularScience");
        }
        try {

            mAlbums = mStoryBooksPresenter.loadData(mPage, "842");
    ...
    public View initSuccessView() {
        try {
            ...
            mTotalCount = (int) SharedPreferencesUtils.getParam(MyApplication.sInstance, "PopularScience_totalCount", 0);
            initListener();
            mPage++;
    ...
    public void onSuccess(List<Album> alba, int totalCount) {
        ...
        SharedPreferencesUtils.setParam(MyApplication.sInstance, "PopularScience_totalCount", mTotalCount);
        if (mAlbums == null) {
            //去更新fragmen UI
            mAlbums = alba;
...
//                mStoryBooksPresenter.loadDataNet(mPage,"842");
...
//        }
    ...
    public void onResume() {
        ...
        MobclickAgent.onPageStart("PopularScienceFramgent"); //统计页面，"MainScreen"为页面名称，可自定义
    }

    public void onPause() {
        super.onPause();
        MobclickAgent.onPause(getActivity());
        MobclickAgent.onPageEnd("PopularScienceFramgent");
    }
}
